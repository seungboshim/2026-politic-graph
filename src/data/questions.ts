...
export const RIGHT_ANTI: Question[] = [
  {
    ...
    options: [
      { label: '단순한 행정 착오와 부실 관리일 뿐이며, 이를 부정선거 음모론으로 몰고 가선 안 된다.', fraud: -35, axes: { trust: 20 } },
      { label: '관리 부실은 분명 비판받아야 하지만, 조직적 부정선거 주장과는 명확히 선을 그어야 한다.', fraud: -10 },
      { label: '조작이라 단정할 순 없으나 의혹이 큰 만큼, 불신 해소를 위한 재검증과 감사는 필요하다.', fraud: 20, axes: { trust: -10 } },
      { label: '단순 실수가 아니라 조직적 조작의 개연성이 높으므로 투표 전면 재검증과 특검이 필요하다.', fraud: 35, axes: { trust: -20 } },
  ...
  },
  {
    ...
    options: [
      { label: '체제가 썩었으므로 광장에서 강력한 아스팔트 구국 투쟁으로 정권을 압박해야 한다.', fraud: 35, axes: { engage: 25, social: 15 } },
      { label: '초기 취지는 공감하나 지나치게 극우 음모론화되어 중도층이 다 이탈해 부담스럽다.', fraud: 0 },
...
export const RIGHT_COMMON: Question[] = [
  ...
  },
  {
    ...
    options: [
      { label: '선관위 사태 정면 돌파 등 야권에 맞서 선방하고 있으므로 현 체제를 적극 지지해야 한다.', fraud: 10, axes: { engage: 20 } },
